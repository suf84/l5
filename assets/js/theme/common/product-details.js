...
import AddToCartUpsell from '../custom/add-to-cart-upsell';
import getProductList from '../custom/parse-csv';
import upsellSuiteCPU from '../custom/upsell-array-cart-page';
import pdpFakeOptions from "../custom/pdp-fake-options";
...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        pdpFakeOptions(context);

        customProductTabs($('.productView-description'));

        // Upsell suite
        const currentProductID = $('[name="product_id"]', $form).val();
        getProductList('product', currentProductID)
            .then(csvProducts => {
                window.upsellProductList = csvProducts;
            })
            .catch(async err => {
                try {
                    let csvProducts = await getProductList('def');
                    window.upsellProductList = csvProducts.filter(prod => prod.product_id != currentProductID);
                }   catch(err) {
                    throw err;
                }
            })
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', (err, response) => {
            ...
            if (this.addToCartUpsell) this.addToCartUpsell.loadUpsellTargets();
    ...
    addProductToCart(event, form) {
         ...
         */
        upsellSuiteCPU.saveUpsellData(
            $('.form input[name="product_id"]').val(),
            this.context.cartItems.map(item => item.product_id),
            Array.from($('.productView-info-value.cpu-product')).map(el => el.innerText)
        );
    ...
    getCartContent(cartItemId, onComplete) {
        const modalTemplate =
            (this.addToCartUpsell && this.addToCartUpsell.hasItems())
                ? 'custom/add-to-cart-upsell-preview'
                : 'cart/preview';
    ...
    updateCartContent(modal, cartItemId, onComplete) {
        this.getCartContent(cartItemId, (err, response) => {
             ...
             */
            if (this.addToCartUpsell && this.addToCartUpsell.hasItems()) {
                this.addToCartUpsell.displayProducts();
            }
